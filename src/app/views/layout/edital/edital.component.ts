...
export class EditalComponent implements OnInit {
   ...
   excluirEdital(){
      this.alertas.warningAlert("Deseja Excluir esse edital?")
         .then((resultado) =>{
            if(resultado.value){
               this.editalService.excluirEdital(Number(this.editalId))
               .subscribe(()=> {
                  this.router.navigate([""])
               })
            }
   ...
   excluirAnexo() {
      ...
      this.cadastrarEditalForm.get('arquivo').setValue(null);
